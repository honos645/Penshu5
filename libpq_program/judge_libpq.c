...
//TODO: エラー2
  ...
  if(result == 0) {
    ...
    for (int i = 0; i < sizeof(creditList) / sizeof(*creditList); i++) {
      ...
      if(creditList[i][0] % 10 == 0 && creditList[i][3] == 0) {
        int class = creditList[i][0];
        for (i; creditList[i][0] < class + 10; i++) {
