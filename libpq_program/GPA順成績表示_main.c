...
int main(int argc, char *argv[]){
  ...
  char department; //表示する学部
  char major; //表示する学科
  int school_year; //表示する学年
  char *name; //生徒の名前
  int sort; //何順で表示するか判断
  char *id; //生徒の学籍番号
  int i; 
  ...
  scanf("%s %s %s %d", &User_Info.level, &User_Info.department, &User_Info.major, &User_Info.school_year);
  printf("GPAを閲覧したい学部・学科・学年を入力してください\n");
  scanf("%s %s %d", &department, &major, &school_year);
  ...
  if( User_Info.level == '4' || User_Info.level == '5'){
  ...
  if( sort == 0 ){
    sprintf(sql, "SELECT b.id, a.person_name, b.gpa, b.grade_ranking FROM users AS a INNER JOIN grade_judge AS b ON a.id = b.id WHERE department = '%c' AND major = '%c' AND school_year = '%d' ORDER BY b.id", department, major, school_year );
  }else if( sort == 1 ){
    sprintf(sql, "SELECT b.id, a.person_name, b.gpa, b.grade_ranking FROM users AS a INNER JOIN grade_judge AS b ON a.id = b.id WHERE department = '%c' AND major = '%c' AND school_year = '%d' ORDER BY b.grade_ranking", department, major, school_year );
  }
  ...
  for(i=0; i<resultRows; i++){
    ...
    gpa = PQgetvalue(res, i, 2); //2 番目のフィールド値を取得
    ranking = PQgetvalue(res, i, 3); //3 番目のフィールド値を取得
    printf("%s %s %s %s\n", id, name, gpa, ranking);
  }
