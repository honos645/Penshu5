...
int personal_grade(pthread_t __selfId, PGconn *con, int __soc, char *__recvBuf, UserInfo *__User_Info)
{
  ...
  char q_id[SMLLBUF];
  char subject_name[BUFSIZE][SMLLBUF], rating[BUFSIZE][SMLLBUF];
  ...
  if (__User_Info->user_level != ADMIN || __User_Info->user_level != CLERK || __User_Info->user_level != TEACH_COM || __User_Info->user_level != CHAIR)
  {
    ...
    else if (__User_Info->user_level == TEACH_HR || __User_Info->user_level == TEACH_VHR && !(__User_Info->school_year == sql_stu_year && strcmp(__User_Info->department, sql_stu_department) == 0 && __User_Info->major == sql_stu_major && __User_Info->school_year == sql_stu_year))
    {
  ...
  sprintf(sql, "SELECT sg.id, sg.subject_code, sg.opening_year, sd.subject_name, sd.credit, sg.grade_point, sg.retake "
               ...
               "AND sg.opening_year = sd.opening_year WHERE id = '%s' ;",
          q_id);
  ...
  sendLen = sprintf(sendBuf, "GPA : %1.2lf\n", gpa);
  printf("%s", sendBuf);
  send(__soc, sendBuf, sendLen, 0);
  printf("[C_THREAD %ld] SEND=> %s", __selfId, sendBuf);
